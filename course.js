...
const completedIndex = 8;
console.log(modules[completedIndex]);

const allModulesDurations = modules.map(module => convertToMin(module.duration));
const completedModulesDurations = allModulesDurations.slice(0, completedIndex + 1);
const completedPct = (getTotalDuration(completedModulesDurations) * 100 / getTotalDuration(allModulesDurations)).toFixed(1);

document.getElementById('total').textContent = convertToHrsMin(getTotalDuration(allModulesDurations));
document.getElementById('done').textContent = convertToHrsMin(getTotalDuration(completedModulesDurations));
document.getElementById('progress').textContent = completedPct + "%";
document.getElementById('progressbar').setAttribute('value', completedPct);
